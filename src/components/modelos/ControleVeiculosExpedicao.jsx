import { Loader2, FileText, FileSpreadsheet } from 'lucide-react';
import { formatCuritiba } from '@/lib/curitiba-time.js';
import { exportTableToPDF, exportTableToExcel } from '@/lib/modelo-export-utils';

const HEADERS = [
  'Data', 'Placa', 'Nome', 'CPF', 'Empresa', 'Ajudante', 'CPF',
  'Horário de Entrada', 'Horário de Saída', 'Classificação', 'Observação'
];

const COL_WEIGHTS = [16, 16, 30, 22, 24, 24, 22, 22, 22, 18, 36];

function formatData(iso) {
  if (!iso) return '';
  return formatCuritiba(iso, { day: '2-digit', month: '2-digit', year: 'numeric' });
}

function formatHora(iso) {
  if (!iso) return '';
  return formatCuritiba(iso, { hour: '2-digit', minute: '2-digit' });
}

function buildObservacao(log) {
  const parts = [];
  if (log.tipo === 'saida') {
    parts.push(log.carregado ? 'Saiu carregado' : 'Saiu vazio');
  }
  if (log.observacao) {
    const obsParts = log.observacao.split(' | ');
    const exitObs = obsParts.length > 1 ? obsParts[obsParts.length - 1] : (log.tipo === 'saida' && !obsParts[0].startsWith('Acompanhante:') ? log.observacao : '');
    if (exitObs) parts.push(exitObs);
  }
  return parts.join(' — ');
}

function buildRow(log) {
  return [
    formatData(log.created_date),
    log.veiculo_placa || '',
    log.motorista_nome || '—',
    log.motorista_cpf || '—',
    log.empresa || '—',
    log.ajudante_nome || '—',
    log.ajudante_cpf || '—',
    formatHora(log.created_date),
    log.tipo === 'saida' ? formatHora(log.data_aprovacao) : '',
    '',
    buildObservacao(log),
  ];
}

export default function ControleVeiculosExpedicao({ logs, loading }) {
  const handlePDF = () => {
    exportTableToPDF({
      title: 'Controle de Veículos Expedição',
      headers: HEADERS,
      rows: logs.map(buildRow),
      columnWeights: COL_WEIGHTS,
      fileName: 'controle_veiculos_expedicao.pdf',
    });
  };

  const handleExcel = () => {
    exportTableToExcel({
      headers: HEADERS,
      rows: logs.map(buildRow),
      fileName: 'controle_veiculos_expedicao.xlsx',
      sheetName: 'Expedição',
    });
  };

  return (
    <div className="print-area bg-card rounded-2xl border border-border p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4 no-print">
        <h2 className="text-center font-heading font-bold text-lg">Controle de Veículos Expedição</h2>
        {!loading && logs.length > 0 && (
          <div className="flex gap-2">
            <button onClick={handlePDF} className="h-9 px-3 rounded-xl bg-secondary text-secondary-foreground text-xs font-medium hover:bg-secondary/80 flex items-center gap-1.5">
              <FileText className="h-4 w-4" /> PDF
            </button>
            <button onClick={handleExcel} className="h-9 px-3 rounded-xl bg-secondary text-secondary-foreground text-xs font-medium hover:bg-secondary/80 flex items-center gap-1.5">
              <FileSpreadsheet className="h-4 w-4" /> Excel
            </button>
          </div>
        )}
      </div>
      {loading ? (
        <div className="text-center py-8"><Loader2 className="h-6 w-6 animate-spin mx-auto text-primary" /></div>
      ) : logs.length === 0 ? (
        <p className="text-center py-8 text-sm text-muted-foreground">Nenhum registro encontrado no período selecionado</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-[10px] border-collapse table-fixed">
            <thead>
              <tr>
                {HEADERS.map(h => (
                  <th key={h} className="border border-black px-1.5 py-1.5 bg-primary text-primary-foreground font-medium text-left leading-tight">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {logs.map(log => (
                <tr key={log.id}>
                  <td className="border border-black px-1.5 py-1 whitespace-nowrap align-top">{formatData(log.created_date)}</td>
                  <td className="border border-black px-1.5 py-1 whitespace-nowrap font-medium align-top">{log.veiculo_placa}</td>
                  <td className="border border-black px-1.5 py-1 break-words align-top">{log.motorista_nome || '—'}</td>
                  <td className="border border-black px-1.5 py-1 whitespace-nowrap align-top">{log.motorista_cpf || '—'}</td>
                  <td className="border border-black px-1.5 py-1 break-words align-top">{log.empresa || '—'}</td>
                  <td className="border border-black px-1.5 py-1 break-words align-top">{log.ajudante_nome || '—'}</td>
                  <td className="border border-black px-1.5 py-1 whitespace-nowrap align-top">{log.ajudante_cpf || '—'}</td>
                  <td className="border border-black px-1.5 py-1 whitespace-nowrap align-top">{formatHora(log.created_date)}</td>
                  <td className="border border-black px-1.5 py-1 whitespace-nowrap align-top">{log.tipo === 'saida' ? formatHora(log.data_aprovacao) : ''}</td>
                  <td className="border border-black px-1.5 py-1 align-top"></td>
                  <td className="border border-black px-1.5 py-1 break-words align-top">{buildObservacao(log)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}