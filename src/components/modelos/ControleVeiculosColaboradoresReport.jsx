...
const COLAB_HEADERS = [
  'PLACA', 'NOME', 'MATRÍCULA', 'SETOR',
  'MODELO DO VEÍCULO', 'COR', 'CNH', 'OBS'
];

const COLAB_COL_WEIGHTS = [16, 30, 18, 20, 24, 14, 22, 36];

function buildColabRow(reg) {
  return [
    reg.placa || '',
    reg.nome || '',
    reg.matricula || '',
...
const FILTER_FIELDS = [
  ...
  { key: 'modelo_veiculo', label: 'Modelo do Veículo' },
  { key: 'cor', label: 'Cor' },
];
...
export default function ControleVeiculosColaboradoresReport() {
  ...
  const loadAndGerar = async () => {
    setLoading(true);
    try {
      const all = await base44.entities.VeiculoColaborador.list('-created_date', 2000);
      setRegistros(all);
      setGenerated(true);
    } catch (e) {}
    setLoading(false);
            ...
            <div className="flex gap-2">
              <button
                onClick={() => exportTableToPDF({
                  title: 'Controle de Veículos — Colaboradores',
                  headers: COLAB_HEADERS,
                  rows: filtered.map(buildColabRow),
                  columnWeights: COLAB_COL_WEIGHTS,
                  fileName: 'controle_veiculos_colaboradores.pdf',
                })}
              ...
              <button
                onClick={() => exportTableToExcel({
                  headers: COLAB_HEADERS,
                  rows: filtered.map(buildColabRow),
                  fileName: 'controle_veiculos_colaboradores.xlsx',
                  sheetName: 'Veículos Colaboradores',
                })}
                className="h-10 px-3 rounded-xl bg-secondary text-secondary-foreground text-sm font-medium hover:bg-secondary/80 flex items-center gap-1.5"
              >
        ...
        <div className="print-area bg-card rounded-2xl border border-border p-6 shadow-sm">
          ...
          <div className="flex items-start justify-between mb-4 border-b-2 border-black pb-3">
            ...
            <div className="flex-1 text-center">
              <h2 className="font-heading font-bold text-sm text-black">CONTROLE DE VEÍCULOS — COLABORADORES</h2>
            </div>
          </div>
                ...
                <tbody>
                  {filtered.map(reg => (
                    <tr key={reg.id} className="hover:bg-muted/20">
                      <td className="border border-black px-1.5 py-1 whitespace-nowrap font-medium align-top">{reg.placa || '—'}</td>
                      <td className="border border-black px-1.5 py-1 break-words align-top">{reg.nome || '—'}</td>
                      <td className="border border-black px-1.5 py-1 whitespace-nowrap align-top">{reg.matricula || '—'}</td>
                      <td className="border border-black px-1.5 py-1 break-words align-top">{reg.setor || '—'}</td>
                      <td className="border border-black px-1.5 py-1 break-words align-top">{reg.modelo_veiculo || '—'}</td>
                      <td className="border border-black px-1.5 py-1 whitespace-nowrap align-top">{reg.cor || '—'}</td>
                      <td className="border border-black px-1.5 py-1 whitespace-nowrap align-top">{reg.cnh || '—'}</td>
                      <td className="border border-black px-1.5 py-1 break-words align-top">{reg.obs || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
