...
import { Loader2, Settings2, Check, DollarSign, Calendar, Route, TrendingUp } from 'lucide-react';
...
export default function EconomicAnalysis({ filialId, filialNome }) {
  const [records, setRecords] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedMes, setSelectedMes] = useState('');
  const [visibleCharts, setVisibleCharts] = useState(() => {
    try { const s = localStorage.getItem('economic_dash_charts'); return s ? JSON.parse(s) : CHART_DEFS.map(c => c.id); }
    catch { return CHART_DEFS.map(c => c.id); }
  });
  const [showSelector, setShowSelector] = useState(false);

  useEffect(() => { loadRecords(); }, [filialId]);
  useEffect(() => { localStorage.setItem('economic_dash_charts', JSON.stringify(visibleCharts)); }, [visibleCharts]);
  ...
  let filialRecords = filialId ? records.filter(r => r.filial_id === filialId) : records;

  let filtered = filialRecords;
  ...
  if (filtered.length === 0) return (
    <div className="bg-card rounded-2xl border border-border p-8 text-center shadow-sm">
      ...
      <p className="text-sm text-muted-foreground">Nenhum dado econômico encontrado{filialNome ? ` para ${filialNome}` : ''}.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h3 className="font-heading font-bold text-lg">Análise Econômica{filialNome ? ` — ${filialNome}` : ''}</h3>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-card rounded-2xl border border-border p-4 shadow-sm flex items-center gap-3">
          ...
          <div className="min-w-0"><p className="text-sm font-bold truncate">R$ {formatCurrency(totalGeral)}</p><p className="text-[10px] text-muted-foreground">Total até a data</p></div>
        </div>
        <div className="bg-card rounded-2xl border border-border p-4 shadow-sm flex items-center gap-3">
          ...
          <div className="min-w-0"><p className="text-sm font-bold truncate">{bestMonth ? bestMonth.mes : '—'}</p><p className="text-[10px] text-muted-foreground">Melhor mês{bestMonth ? ` · R$ ${formatCurrency(bestMonth.valor)}` : ''}</p></div>
        </div>
        <div className="bg-card rounded-2xl border border-border p-4 shadow-sm flex items-center gap-3">
        ...
        <div className="bg-card rounded-2xl border border-border p-4 shadow-sm flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-green-500/10 flex items-center justify-center shrink-0"><TrendingUp className="h-5 w-5 text-green-600" /></div>
          <div className="min-w-0"><p className="text-sm font-bold truncate">{filtered.length}</p><p className="text-[10px] text-muted-foreground">Registros de averbação</p></div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        {visibleCharts.includes('topRoutes') && topRoutes.length > 0 && (
          <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
            <h4 className="font-heading font-bold text-sm mb-3">Top 10 Rotas/Prioridades por Receita</h4>
            <ResponsiveContainer width="100%" height={220}>
        ...
        {visibleCharts.includes('trend') && monthlyData.length > 0 && (
          <div className="bg-card rounded-2xl border border-border p-5 shadow-sm md:col-span-2">
            <h4 className="font-heading font-bold text-sm mb-3">Tendência de Receita Mensal</h4>
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={monthlyData}>
                <defs>
                  <linearGradient id="econTrendGrad" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis tick={{ fontSize: 9 }} tickFormatter={(v) => `R$ ${(v / 1000).toFixed(0)}k`} />
                <Tooltip contentStyle={{ borderRadius: 12, border: '1px solid hsl(var(--border))' }} formatter={(v) => `R$ ${formatCurrency(v)}`} />
                <Area type="monotone" dataKey="valor" stroke="hsl(var(--primary))" fill="url(#econTrendGrad)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
